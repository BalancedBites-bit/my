...
async function sendPaymentConfirmationEmail(
  ...
  <body>
    ...
    attachments: [{ filename: name, content: compressedImageBuffer }],
  };
  try {
...
const sendCounselingSessionRequest = (request) => {
  const mailOptions = {
    from: request.email,
    to: process.env.email,
...
const sendCounselingSessionAcceptance = (request) => {
  const mailOptions = {
    from: process.env.email,
    to: request.email,
    subject: "Counseling Session Accepted",
    html: `
        <div style="font-family: Arial, sans-serif; line-height: 1.6;">
...
const sendCounselingSessionRejection = (request) => {
  const mailOptions = {
    from: process.env.email,
    to: request.email,
    subject: "Counseling Session Request Rejected",
    html: `
        <div style="font-family: Arial, sans-serif; line-height: 1.6;">
...
const sendDietPlanRequest = (request, userData) => {
  const mailOptions = {
    from: userData.email,
    to: process.env.email,
...
const sendDietPlanAcceptance = (request, userData) => {
  const mailOptions = {
    from: process.env.email,
    to: userData.email,
    subject: "Diet Plan Accepted",
    html: `
        <div style="font-family: Arial, sans-serif; line-height: 1.6;">
...
const sendDietPlanRejection = (request, userData) => {
  const mailOptions = {
    from: process.env.email,
    to: userData.email,
    subject: "Diet Plan Request Rejected",
    html: `
        <div style="font-family: Arial, sans-serif; line-height: 1.6;">
...
function sendContactEmail(email, message) {
  const htmlContent = `
    <!DOCTYPE html>
    <html lang="en">
    <head>
...
module.exports = {
  sendOTP,
  sendCounselingSessionRequest,
  sendCounselingSessionAcceptance,
  sendCounselingSessionRejection,
  sendPaymentConfirmationEmail,
  sendDietPlanRequest,
  sendDietPlanAcceptance,
  sendDietPlanRejection,
  sendContactEmail,
};
